...
fn main() {
    ...
    let my_color = color(1.0, 0.0, 0.0);
    let light_position = Tuple::set_point(-10.0, 10.0, -10.0);
    ...
    test_sphere.material.color = color(1.0, 0.2, 1.0);
    for y in 0..(canvas_size - 1) {
        ...
        for x in 0..(canvas_size - 1) {
            ...
            if xs.len() > 0 {
                ...
                let current_color = lighting(
                    test_sphere.material.clone(),
                    light.clone(),
                    hit_position.clone(),
                    eye_vector.clone(),
                    normal.clone(),
